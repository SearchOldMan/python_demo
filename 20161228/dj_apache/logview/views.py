...
def main(request):
	...
	return render_to_response('logview/main.html',{'os_list':os_list,'svc_list':svc_list,'hardware_list':hardware_list})
...
def categorized(request,category,category_id):
	...
	if category == 'os':
		...
		server_list = models.Server.objects.filter(server__exact=category_id)
		category_name = models.Service.objects.get(id=category_id)
	elif category == 'hw':
		server_list = models.Server.objects.filter(hardware_component_exact=category_id)
		category_name = models.HardwareComponent.objects.get(id=category_id)
	else:
		server_list = []
	return render_to_response('logview/categorized.html',{'server_list':server_list,'category':category_dict[category],'category_name':category_name})


def server_detail(request,server_id):
	server = models.Server.objects.get(id=server_id)
	return render_to_response('logview/server_detail.html',{'server':server})